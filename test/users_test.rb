...
class UsersTest < ActiveSupport::TestCase
  ...
  def test_creates_a_user
    post '/users', { username: 'Alan', password: 'Ala1Ma2Kota' }.to_json, { "CONTENT_TYPE" => "application/json" }
    assert_equal 201, last_response.status
  end

  def test_fails_to_create_a_user_without_username
    post '/users', { password: 'Ala1Ma2Kota' }.to_json, { "CONTENT_TYPE" => "application/json" }
