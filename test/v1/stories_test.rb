...
class StoriesTest < ActiveSupport::TestCase
  ...
  def test_submitting_a_new_story
    ...
    post '/v1/stories', { title: 'Lorem epsum', url: 'http://www.lorem.com' }.to_json,
                     { "CONTENT_TYPE" => "application/json" }
    assert_equal 201, last_response.status

    assert_equal '/v1/stories', last_response.original_headers['Location']
    data = JSON.parse last_response.body
    assert data['id'] != nil
  end
  ...
  def test_submitting_new_story_in_xml_format
    authorize @username, @password
    header 'Accept', 'application/xml'
    post '/v1/stories', { title: 'Lorem epsum', url: 'http://www.lorem.com' }.to_xml,
  ...
  def test_submitting_new_story_fails_when_title_is_missing
    authorize @username, @password
    post '/v1/stories', { url: 'http://www.lorem.com' }.to_json,
                     { "CONTENT_TYPE" => "application/json" }
  ...
  def test_updating_a_story
    authorize @username, @password
    put "/v1/stories/#{@story1.id}", { id: @story1.id, url: 'http://www.l.com' }.to_json,
                                  { "CONTENT_TYPE" => "application/json" }
  ...
  def test_upvoting_a_story
    authorize @username, @password
    patch '/v1/stories/1/vote', { delta: 1 }.to_json, { "CONTENT_TYPE" => "application/json" }
  ...
  def test_second_upvoting_doesnt_affect_story
    authorize @username, @password
    patch "/v1/stories/#{@story1.id}/vote", { delta: 1 }.to_json, { "CONTENT_TYPE" => "application/json" }
    patch "/v1/stories/#{@story1.id}/vote", { delta: 1 }.to_json, { "CONTENT_TYPE" => "application/json" }
    get "/v1/stories/#{@story1.id}"
  ...
  def test_undoing_a_vote
    ...
    delete "/v1/stories/#{@story1.id}/vote"
    assert_equal 204, last_response.status
    assert_equal '', last_response.body
